...
while [ $PP_EXIT -gt 0 ] && [ $NUM_ATTEMPT -lt 5 ] ; do
	...
	add_basecall_align_update "Starting chr-wise samtools merge using parallel utility" $LOG_FILE
...
if [ ${PP_EXIT} -gt 0 ]; then
	...
	add_basecall_align_update "Parallel chr-wise samtools merge jobs exited with non-zero code $PP_EXIT even after 5 attempts, exiting job" $LOG_FILE
	email_basecall_align_update "POSTPROCESS STATUS: Parallel chr-wise samtools merge jobs unsuccessful" $LOG_FILE $ERROR_EMAIL_SUB 
	exit 21

else

	add_basecall_align_update "Parallel chr-wise samtools merge jobs exited successfully in $NUM_ATTEMPT attempt/s" $LOG_FILE
...
if [ ${PP_EXIT} -gt 0 ]; then
	...
	add_basecall_align_update "Parallel chr-wise bam upload jobs exited with non-zero code $PP_EXIT even after 5 attempts, exiting job" $LOG_FILE
	email_basecall_align_update "POSTPROCESS STATUS: Parallel chr-wise samtools upload jobs unsuccessful" $LOG_FILE $ERROR_EMAIL_SUB 
	exit 22
