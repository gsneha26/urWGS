...
for z in $(gcloud compute machine-types list | grep n1-standard-96 | grep 'us-' | sort -V | awk '{print $2}'); do
  ...
  gcloud compute instances create $1 \
    ...
    --metadata CHR=$2,CONFIG_FILE_URL=$3,startup-script='#!/bin/bash
      ...
      for i in $( echo $CHR | sed s/:/ /g ); do
          echo "2" > /data/spectre_status/${i}_spectre_status.txt 
      done
      echo "2" > /data/spectre_status.txt 
      chmod a+w -R /data/
      chmod +x $PROJECT_DIR/*/*.sh
      echo -e "SHELL=/bin/bash\nPATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/snap/bin\nPROJECT_DIR=$PROJECT_DIR\n*/1 * * * * bash -c $PROJECT_DIR/spectre/run_spectre_pipeline_wrapper.sh >> /data/stdout.log 2>> /data/stderr.log" | crontab -'
