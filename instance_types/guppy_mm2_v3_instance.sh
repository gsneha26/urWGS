...
#FILTER_GUPPY=$(echo "attributes.FC=\""$2"\" AND attributes.STAGE=\"guppy\"")
#FILTER_POSTPROCESS=$(echo "attributes.FC=\""$2"\" AND attributes.STAGE=\"guppy\"")
#gcloud pubsub subscriptions create ${1}_sub_guppy \
#	--topic=${3} \
#	--message-filter="${FILTER_GUPPY}"
#gcloud pubsub subscriptions create ${1}_sub_postprocess \
#	--topic=${3} \
#	--message-filter="${FILTER_POSTPROCESS}"
gcloud compute instances create $1 \
	...
	--local-ssd=interface=NVME \
		...
		gsutil cp gs://ultra_rapid_nicu/scripts/sample.config /data/
                mkdir /data/scripts
                gsutil -m cp gs://ultra_rapid_nicu/scripts/guppy_mm2_v3/* /data/scripts/
