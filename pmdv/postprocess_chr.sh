...
gsutil -o "GSUtil:parallel_composite_upload_threshold=750M" -m cp $CHR_FOLDER/margin/MARGIN_PHASED.PEPPER_SNP_MARGIN.haplotagged.bam ${HP_BAM_BUCKET}/${SAMPLE}_$1.bam
if [ $? -gt 0 ]; then
	1>&2 echo "Error with uploading haplotagged bam for chr"$1
	exit 1
fi

gsutil -o "GSUtil:parallel_composite_upload_threshold=750M" -m cp $CHR_FOLDER/margin/MARGIN_PHASED.PEPPER_SNP_MARGIN.haplotagged.bam.bai ${HP_BAM_BUCKET}/${SAMPLE}_$1.bam.bai
if [ $? -gt 0 ]; then
