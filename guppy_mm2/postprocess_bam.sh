...
EMAIL_FILE=/data/logs/postprocess.log
...
while [ $PP_EXIT -gt 0 ] && [ $NUM_ATTEMPT -lt 5 ] ; do
	...
	add_guppy_mm2_update "Starting chr-wise samtools merge using parallel utility" $EMAIL_FILE
...
if [ ${PP_EXIT} -gt 0 ]; then
	...
	add_guppy_mm2_update "Parallel chr-wise samtools merge jobs exited with non-zero code $PP_EXIT even after 5 attempts, exiting job" $EMAIL_FILE
	email_guppy_mm2_update "POSTPROCESS STATUS: Parallel chr-wise samtools merge jobs unsuccessful" $EMAIL_FILE $ERROR_EMAIL_SUB 
	exit 21

else

	add_guppy_mm2_update "Parallel chr-wise samtools merge jobs exited successfully in $NUM_ATTEMPT attempt/s" $EMAIL_FILE
...
while [ $PP_EXIT -gt 0 ] && [ $NUM_ATTEMPT -lt 5 ] ; do
	...
	add_guppy_mm2_update "Starting chr-wise bam upload using parallel utility" $EMAIL_FILE
...
if [ ${PP_EXIT} -gt 0 ]; then
	...
	add_guppy_mm2_update "Parallel chr-wise bam upload jobs exited with non-zero code $PP_EXIT even after 5 attempts, exiting job" $EMAIL_FILE
	email_guppy_mm2_update "POSTPROCESS STATUS: Parallel chr-wise samtools upload jobs unsuccessful" $EMAIL_FILE $ERROR_EMAIL_SUB 
	exit 22
